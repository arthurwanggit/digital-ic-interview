...
| 3. 编码实战 | 按面试节奏掌握 | 热身代码分析 + 按方向现场写码 + 通用追问（题库：设计题库A） |
| 4. 开放设计 (To be Done) | — | 此阶段内容待补充 |
...
**题库关联：** 简历命中知识点11-17（后端/物理设计）相关关键词时，项目题可从 `题库/后端题库A.txt`（若相对路径失效，按文件名在工程内搜索）中选题，英文选择题须转述为**中文开放题**提问（隐去选项，只问考点本身），追问仍按各知识点路线表进行。
...
题库文件：`题库/设计题库A.txt`（若相对路径失效，按文件名在工程内搜索）。
流程：热身 → 主菜 → 追问。

### 热身（所有方向通用，读代码分析，不写码）

出示第1题中方法一/方法二两段无注释 Verilog 伪代码，依次提问：

1. 问题1：分别解释二者的分频数，并说明判断依据
2. 问题2：二者输出时钟的性质差别与各自适用场景（占空比/边沿相位/毛刺/功耗/后端处理等维度）
3. 追问：如果需要 50% 占空比的分频输出，怎么修改方法二

### 主菜（按候选人项目方向选题，现场写码）

- 做过接口/SPI/I2C/UART/Timer/PWM → 分频器题组，**造进阶**：第2题（参数化偶数分频）→ 答对进第3题（50% 占空比 5 分频）→ 答对进第4题（非 50% 占空比 3 分频）；答错或放弃类回答不进阶，直接转追问层
- 其他方向 → 沿用下方选题框架表（暂无题库文件，面试官自行把握考察要点）：
...
（注：原"做过接口/SPI/I2C → 分频器设计"一行已由题库设计题库A 第2/3/4题承接）

### 追问层（主菜完成后触发，出自题库通用追问）

- 追问1：给电路补上 test_clk、test 两个信号，说明用途并修改代码（test bypass 概念，视候选人水平选问）
- 追问2 分支考察：候选人代码中写了 buf 等实例原语 → 要求写出 SDC；纯 Verilog RTL 写法 → 问综合会遇到什么问题

### 提问规则

与阶段2一致：一次只问一个问题、每题追问1-2层、不评价不科普、放弃类回答立即跳过。

### 评分挂钩

- 主菜 → 「编码能力」维度（代码结构清晰度、可综合意识、corner case 覆盖）
- 热身 + 追问2 → 「基础知识」维度
